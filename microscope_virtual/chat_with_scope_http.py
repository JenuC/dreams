...
def _(mo):
    source_picker = mo.ui.dropdown(
        options=["gradient", "rings", "spectrum"],
        value="gradient",
        label="Image source",
    )
    step_size = mo.ui.number(value=10.0, start=0.1, stop=1000.0, step=1.0, label="Step (µm)")
    ...
    mo.vstack([
        mo.hstack([source_picker, step_size, btn_snap], justify="start"),
        mo.hstack([
            mo.vstack([
                mo.md("**XY**"),
    ...
    return btn_snap, btn_xm, btn_xp, btn_ym, btn_yp, btn_zm, btn_zp, source_picker, step_size


# ── X+ ────────────────────────────────────────────────────────────────────────

@app.cell
async def _(ClientSession, MCP_URL, btn_xp, json, step_size, streamablehttp_client):
...
@app.cell
async def _(ClientSession, MCP_URL, btn_xm, json, step_size, streamablehttp_client):
...
@app.cell
async def _(ClientSession, MCP_URL, btn_yp, json, step_size, streamablehttp_client):
...
@app.cell
async def _(ClientSession, MCP_URL, btn_ym, json, step_size, streamablehttp_client):
...
@app.cell
async def _(ClientSession, MCP_URL, btn_zp, json, step_size, streamablehttp_client):
...
@app.cell
async def _(ClientSession, MCP_URL, btn_zm, json, step_size, streamablehttp_client):
...
def _(mo, pos_xm, pos_xp, pos_ym, pos_yp, pos_zm, pos_zp):
    ...
    mo.callout(
        mo.md(f"**Stage** &nbsp; X: `{_pos['x']:.1f}` &nbsp; Y: `{_pos['y']:.1f}` &nbsp; Z: `{_pos['z']:.1f}` µm"),
        kind="info",
    )
    return
...
@app.cell
async def _(ClientSession, Image, MCP_URL, btn_snap, json, mo, source_picker, streamablehttp_client):
    _trigger = btn_snap.value
    async with streamablehttp_client(MCP_URL) as (_r, _w, _):
        async with ClientSession(_r, _w) as _s:
            await _s.initialize()
            await _s.call_tool("set_test_image", {"source": source_picker.value})
