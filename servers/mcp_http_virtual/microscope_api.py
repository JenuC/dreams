...
class TestImage(str, Enum):
    RINGS = "rings"        # concentric rings (grayscale-like)
    SPECTRUM = "spectrum"  # RGB color gradient
    GRADIENT = "gradient"  # horizontal grayscale gradient


def _load_test_image(source: TestImage) -> np.ndarray:
    """Load a test image as an (H, W, 3) uint8 RGB array."""
    if source == TestImage.RINGS:
        size = 512
...
class VirtualMicroscope:
    ...
    def __init__(self, test_image: TestImage = TestImage.GRADIENT):
