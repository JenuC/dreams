...
def _():
    ...
    SCRIPT = r"C:\Users\lociuser\code\jenu\dreams\microscope_mcp\server_real.py"
...
@app.cell
async def _(ClientSession, StdioServerParameters, stdio_client):
    server_params = StdioServerParameters(
    ...
    async def fetch_xyz():
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                await session.initialize()
                result = await session.call_tool("get_stage_position", {})
                print("XYZ:", result)
                return result
    #x = 
    xyz = await fetch_xyz()
...
@app.cell
async def _(ClientSession, StdioServerParameters, stdio_client):
    async def fetch_latest_imagexxxx():
        server_params = StdioServerParameters(
            command=r"C:\Users\lociuser\code\jenu\dreams\.venv\Scripts\python.exe",
            args=[r"C:\Users\lociuser\code\jenu\dreams\microscope_mcp\server.py"],
        )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if hasattr(content, "data"):
                    ...
                    return np.asarray(json.loads(content.text))
                ...
                print("MCP content type:", type(content))
                print("Payload size:", len(payload))

                return img
    imgxxxx = await fetch_latest_imagexxxx()
    return (imgxxxx,)
...
@app.cell
async def _(ClientSession, Image, StdioServerParameters, stdio_client):
    async def fetch_latest_imagexxx():
        server_params = StdioServerParameters(
            command=r"C:\Users\lociuser\code\jenu\dreams\.venv\Scripts\python.exe",
            args=[r"C:\Users\lociuser\code\jenu\dreams\microscope_mcp\server.py"],
        )
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if hasattr(content, "text"):
                    import json, numpy as np
                    img = np.asarray(json.loads(content.text))
    ...
    resultxxx = await fetch_latest_imagexxx()
    imgxxx = Image.open(resultxxx)
    imgxxx
    return
